...
addReachable m noob = M.insert noob () m
addNoob g key () m =
    let reachables = fromJust $ lookup key g
    in foldl addReachable m reachables

saturate g m = let m' = M.foldrWithKey (addNoob g) m m in if m' == m then m else saturate g m'

removeNoob noob () g = filter ((/= noob) . fst) g
removegroup g m = M.foldrWithKey removeNoob g m

go :: [(Int, [Int])] -> Int
go g =
    if length g == 0 then 0
    else
        let group0 = saturate g $ M.singleton (fst $ head g) ()
            input1 = traceShow group0 $ removegroup g group0
        in traceShow input1 $ 1 + go input1

-- main = print $ length $ saturate $ M.singleton 0 ()
main = print $ go input
