...
day = map ((head *** sum) . unzip) . groupOn fst . concatMap step

solve n = sum $ map snd $ (iterate day $ map (head &&& length) $ group $ sort input) !! n

main = do print $ solve 80; print $ solve 256
