...
play seen p1 [] = traceShow ("p1",p1) $ (True, score p1)
play seen [] p2 = traceShow ("p2",p2) $ (False, score p2)
-- play seen (p1:p1s) (p2:p2s) = traceShow ("round",p1,p1s,p2,p2s) $ if p1 > p2 then play newseen (p1s ++ [p1,p2]) p2s else play newseen p1s (p2s ++ [p2,p1])
play seen (p1:p1s) (p2:p2s) =
    if length p1s >= p1 && length p2s >= p2 then traceShow ("create subgame",p1s,p2s) $
