...
parseOpcode op = (mod op 100, map parseMode [100,1000,10000])
...
intcode input output program index =
    let (op, [mode1, mode2, mode3]) = parseOpcode $ program !! index
        arg :: Mode -> Int -> Int
        arg mode offset =
    ...
    in case op of
           ...
           1 -> intcode input output newprogram $ index + 4
                    where value = arg mode1 1 + arg mode2 2
                          newprogram = set (arg Immediate 3) value program
           -- mul
           ...
           -- output
           4 -> intcode input (value:output) program $ index + 2
                    where value = arg mode1 1
           -- jump if true
           5 -> intcode input output program newindex
                    where value = arg mode1 1
                          newindex = if value /= 0 then arg mode2 2 else index + 3
           -- jump if false
           6 -> intcode input output program newindex
                    where value = arg mode1 1
                          newindex = if value == 0 then arg mode2 2 else index + 3
           -- less than
           7 -> intcode input output newprogram $ index + 4
                    where value = if arg mode1 1 < arg mode2 2 then 1 else 0
                          newprogram = set (arg Immediate 3) value program
           -- equals
