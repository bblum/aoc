...
data Nobe = Nobe { weight :: Int, ownweight :: Int, children :: [Nobe] } deriving Eq

maketree list name = Nobe (w + sum (map weight ns)) w ns
    where (w,names) = fromJust $ lookup name list
          ns = map (maketree list) names

badnobe n = mplus (msum $ map badnobe $ children n) (if bad then Just n else Nothing)
    where bad = length (nub $ map weight $ children n) > 1

-- part 1 -- figured out which was the root nobe using:
-- for i in `cat input.txt | cut -d' ' -f1`; do grep $i input.txt | wc -l | sed "s/$/ $i/"; done | grep ^1
main = print $ ownweight n + (weight $ fromJust $ find (/= n) ns) - weight n
    where ns = children $ fromJust $ badnobe $ maketree input "svugo"
          n = fromJust $ find (\n -> length (filter ((== weight n) . weight) ns) == 1) ns

input = map (\(x,y,z) -> (x,(y,z))) [
